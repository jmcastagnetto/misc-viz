...
#  geom_vline(xintercept = 1:4, linetype = "dashed", color = "grey") +
  geom_point(size = 8) +
  geom_text(data = world_top10_cases %>% filter(map_val == 1),
            aes(x = map_val - .1, label = country_region),
            size = 7, hjust = 1) +
  geom_text(data = world_top10_cases %>% filter(map_val == 4),
            aes(x = map_val + .1, label = country_region),
            size = 7, hjust = 0) +
  geom_bump(size = 2) +
  scale_y_reverse(
  ...
  labs(
    x = "",
    y = "Rank",
    title = "Worldwide top 10 countries by confirmed COVID-19 cases",
